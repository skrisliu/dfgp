...
class rscls:
    ...
    def locate_sample_reg(self):
        sam = []
        for i in [1]:
            _xy = np.array(np.where(self.gt_b==i)).T
            ys = self.gt2[np.where(self.gt_b==i)]
            _sam = np.concatenate([_xy,ys.reshape([_xy.shape[0],1])],axis=-1)
            try:
                sam = np.concatenate([sam,_sam],axis=0)
            except:
                sam = _sam
